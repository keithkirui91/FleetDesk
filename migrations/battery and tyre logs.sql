...
-- Battery workbook lifespan is labelled HRS while the app schema stores expected_lifespan_months;
-- those HRS values are preserved in notes instead of being loaded into the months column.
...
INSERT INTO battery_change_logs (vehicle_id, service_record_id, change_date, odometer, quantity, battery_size, battery_type, expected_lifespan_months, reason_for_removal, notes)
SELECT v.id, NULL, s.change_date, s.odometer, s.quantity, s.battery_size, s.battery_type, s.expected_lifespan_months, s.reason_for_removal, s.notes
